...
const FallbackForm: React.FC = () => (
  <div className="text-center text-red-500 dark:text-red-400 p-4">
    Error: Form component failed to load. Please check the Form component
    implementation.
  </div>
);
...
const ParticleBackground: React.FC = () => {
  ...
  const particleVariants = {
    animate: (i: number) => ({
      x: [
        particles[i].x,
        particles[i].x + (Math.random() * 60 - 30),
        particles[i].x,
      ],
      y: [
        particles[i].y,
        particles[i].y + (Math.random() * 60 - 30),
        particles[i].y,
      ],
  ...
  return (
    <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
      {particles.map((particle) => (
        <motion.div
          ...
          style={{
            ...
            top: `${particle.y}%`,
            background:
              "linear-gradient(135deg, hsl(215 91% 70% / 0.5), hsl(271 81% 75% / 0.5))",
          }}
      ...
      <motion.div
        ...
        style={{
          background:
            "linear-gradient(45deg, transparent, hsl(215 91% 70% / 0.2), transparent)",
        }}
        animate={{
          opacity: [0.1, 0.2, 0.1],
          x: [-30, 30, -30],
