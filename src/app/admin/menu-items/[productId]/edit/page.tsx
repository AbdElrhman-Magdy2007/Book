...
const cachedGetProducts = cache(async () => {
  ...
  try {
    ...
    if ("error" in result || !Array.isArray(result)) {
      console.error(
        `${LOG_PREFIX} [${requestId}] Failed to fetch products:`,
        result?.error
      );
      return [];
    }
...
const cachedGetCategories = cache(async () => {
  ...
  try {
    ...
    if ("error" in result || !Array.isArray(result)) {
      console.error(
        `${LOG_PREFIX} [${requestId}] Failed to fetch categories:`,
        result?.error
      );
      return [];
    }
...
export async function generateStaticParams() {
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      console.log(
        `${LOG_PREFIX} [${requestId}] Generated params:`,
        params.length
      );
    }
    ...
    console.error(
      `${LOG_PREFIX} [${requestId}] Error in generateStaticParams:`,
      error
    );
    return [];
  }
...
const getProduct = cache(
  async (productId: string): Promise<ProductWithRelations | null> => {
    const requestId = generateRequestId();
    console.log(`${LOG_PREFIX} [${requestId}] Fetching product ID:`, productId);

    if (!productId || typeof productId !== "string") {
      console.warn(
        `${LOG_PREFIX} [${requestId}] ${ERROR_MESSAGES.INVALID_ID} | productId:`,
        productId
      );
      return null;
    }
    ...
    try {
      ...
      if (!product) {
        console.warn(
          `${LOG_PREFIX} [${requestId}] ${ERROR_MESSAGES.PRODUCT_NOT_FOUND} | productId:`,
          productId
        );
      }
      ...
      console.error(
        `${LOG_PREFIX} [${requestId}] Error fetching product:`,
        error,
        "| productId:",
        productId
      );
...
export default async function EditProductPage({
  ...
  if (process.env.NODE_ENV === "development") {
    console.log(
      `${LOG_PREFIX} [${requestId}] Rendering page for product ID:`,
      productId
    );
  }
  ...
  if (!productId || typeof productId !== "string") {
    ...
    return (
      <ErrorAlert
        ...
        description="A valid product ID was not provided. Please go back and select a valid product."
      />
    );
  }
  ...
  if (!product) {
    ...
    return (
      <ErrorAlert
        ...
        description={`The requested product (ID: ${productId}) was not found. It may have been deleted or the link is incorrect. Please return to the product list.`}
      />
    );
  }
  ...
  if (!categories || categories.length === 0) {
    console.error(
      `${LOG_PREFIX} [${requestId}] ${ERROR_MESSAGES.NO_CATEGORIES}`
    );
    return (
      <main
        className={clsx(
      ...
      >
        ...
        <ErrorAlert
          ...
          description={ERROR_MESSAGES.NO_CATEGORIES}
          actionHref={`/${Routes.ADMIN}/${Pages.CATEGORIES}`}
