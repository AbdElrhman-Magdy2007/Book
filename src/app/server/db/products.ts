...
const productInclude = {
  ...
  orders: {
    select: { id: true, quantity: true, productId: true, orderId: true },
  },
...
const sanitizeCategoryName = (name: string): string => {
  return (
    name.replace(/[ممممممممتتتdggfjgjgh]+/g, "").trim() || "Unnamed Category"
  );
};
...
export const getAllProducts = cache(
  async (): Promise<ProductWithRelations[] | ErrorResponse> => {
    ...
    try {
      ...
      return products.length > 0
        ? products
        : {
            error: ERROR_MESSAGES.NO_PRODUCTS,
            code: "NO_PRODUCTS",
            details: "The database contains no products",
          };
    } catch (error) {
      console.error(
        `${LOG_PREFIX} [${requestId}] Error fetching products:`,
        error
      );
      return {
...
export const getProducts = cache(
  ...
  ): Promise<ProductWithRelations[] | ErrorResponse> => {
    const requestId = generateRequestId();
    console.log(
      `${LOG_PREFIX} [${requestId}] Fetching products with options:`,
      options
    );
    ...
    try {
      ...
      return products.length > 0
        ...
        : {
            error: ERROR_MESSAGES.NO_PRODUCTS,
            code: "NO_PRODUCTS",
            details: "No products match the provided criteria",
          };
    } catch (error) {
      console.error(
        `${LOG_PREFIX} [${requestId}] Error fetching products:`,
...
export const getProductsCount = cache(
  async (
    options: Pick<GetProductsOptions, "search"> = {}
  ): Promise<number | ErrorResponse> => {
    const requestId = generateRequestId();
    console.log(
      `${LOG_PREFIX} [${requestId}] Counting products with search:`,
      options.search
    );
    ...
    try {
      ...
      console.error(
        `${LOG_PREFIX} [${requestId}] Error counting products:`,
        error
      );
      return {
        error: ERROR_MESSAGES.FETCH_FAILED,
...
export const getBestSellers = cache(
  async (limit?: number): Promise<ProductWithRelations[] | ErrorResponse> => {
    const requestId = generateRequestId();
    console.log(
      `${LOG_PREFIX} [${requestId}] Fetching best sellers with limit:`,
      limit
    );
    ...
    try {
      ...
      return products.length > 0
        ? products
        : {
            error: ERROR_MESSAGES.NO_BEST_SELLERS,
            code: "NO_BEST_SELLERS",
            details: "No products with orders exist",
          };
    } catch (error) {
      console.error(
        `${LOG_PREFIX} [${requestId}] Error fetching best sellers:`,
        error
      );
      return {
        error: ERROR_MESSAGES.FETCH_FAILED,
...
export const getProductsByCategory = cache(
  async (): Promise<CategoryWithProducts[] | ErrorResponse> => {
    const requestId = generateRequestId();
    console.log(
      `${LOG_PREFIX} [${requestId}] Fetching categories with products`
    );
    ...
    try {
      ...
      console.log(`${LOG_PREFIX} [${requestId}]`, {
        ...
        productCount: validatedCategories.reduce(
      ...
      return validatedCategories.length > 0
        ? validatedCategories
        : {
            error: ERROR_MESSAGES.NO_CATEGORIES,
            code: "NO_CATEGORIES",
            details: "The database contains no categories",
          };
    } catch (error) {
      console.error(
        `${LOG_PREFIX} [${requestId}] Error fetching categories:`,
        error
      );
      return {
        error: ERROR_MESSAGES.FETCH_FAILED,
...
export const getProductsByCategoryId = cache(
  async (
    ...
    const requestId = generateRequestId();
    console.log(
      `${LOG_PREFIX} [${requestId}] Fetching products for category ID: ${categoryId}`
    );
    ...
    try {
      ...
      return products.length > 0
        ...
        : {
            error: ERROR_MESSAGES.NO_PRODUCTS,
            code: "NO_PRODUCTS",
            details: `No products found for category ID ${categoryId}`,
          };
    } catch (error) {
      console.error(
        `${LOG_PREFIX} [${requestId}] Error fetching products by category:`,
        error
      );
      return {
        error: ERROR_MESSAGES.FETCH_FAILED,
